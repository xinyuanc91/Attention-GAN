...
class CycleAttnGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        # attn constrain
        lambda_sparse_attn_A = self.opt.loss_attn_A
        lambda_sparse_attn_B= self.opt.loss_attn_B
        #
        self.loss_attnsparse_A = self.criterionIdt(self.attn_real_A, self.zeros_attn) * lambda_sparse_attn_A
        self.loss_attnsparse_B = self.criterionIdt(self.attn_real_B, self.zeros_attn) * lambda_sparse_attn_B
        self.loss_attnconst_A = self.criterionIdt(self.attn_fake_A, self.attn_real_B.detach())
        self.loss_attnconst_B = self.criterionIdt(self.attn_fake_B, self.attn_real_A.detach())
        # combined loss
    ...
    def get_current_errors(self):
        ...
        attn_A = self.loss_attnconst_real_A.data
        attn_B = self.loss_attnconst_real_B.data
        if self.opt.identity > 0.0:
            ...
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ('Cyc_A', Cyc_A), ('attn_A', attn_A),
                                ('D_B', D_B), ('G_B', G_B), ('Cyc_B', Cyc_B), ('attn_B', attn_B)])
